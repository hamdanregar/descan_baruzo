    ...
    <aside className="w-64 bg-[#f8fafc] border-r border-slate-200 h-screen sticky top-0 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-6">
        {menus.map((g) => (
          <div key={g.group}>
            ...
            {g.items.map((item) => (
              <Link key={item.name} href={item.href} className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-white hover:text-blue-600 rounded-lg transition-all mb-1 group shadow-sm hover:shadow-md border border-transparent hover:border-slate-100">
      ...
      <div className="p-4 border-t border-slate-200">
        <button
          ...
          className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-all w-full"
        >
